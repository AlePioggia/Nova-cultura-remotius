...
export class AuthenticationService {
  ...
  async logIn(authenticationRequest: IAuthenticationRequest) {
    const response = await this.helperService.post(this.baseUrl + "sign-user", authenticationRequest);
  ...
  //metodo statico
  getRoles() {
    const roles: Role[] = [{ id: 0, role: 'student' }, { id: 1, role: 'teacher' }];
    return roles;
  }
  ...
  isTeacher(): boolean {
    const user: ICreateUserRequest = this.getUser(sessionStorage.getItem("access_token"));
